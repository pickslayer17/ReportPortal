...
namespace ReportPortal.Controllers
{
    ...
    public class TestResultManagementController : ControllerBase
    {
        ...
        [Authorize]
        public async Task<IActionResult> AddTestResult(int testId, [FromBody] TestResultVm testVm, CancellationToken cancellationToken = default)
        {
            var testResultDto = _mapper.Map<TestResultDto>(testVm);
            var testResultCreated = await _testResultService.AddTestResultToTestAsync(testId, testResultDto, cancellationToken);
